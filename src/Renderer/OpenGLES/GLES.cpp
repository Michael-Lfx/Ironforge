...
struct GLExtensionSupport
{
    ...
    uint32_t mHasShaderBinaryOES : 1;//TODO 没啥意义，android 支持率低
    uint32_t mHasProgramBinaryOES : 1;
};
...
{
    GLint	  mIndex;//存储 glGetAttribLocation 返回值
    GLint     mSize;
    GLenum    mType;
    GLboolean mNormalized;
    GLsizei   mStride;
    GLuint	  mOffset;//用户配置 VertexAttrib::mOffset
} GlVertexAttrib;
...
{
    uint32_t mId;//意义是什么
    uint32_t mVAO;//VAO gl id
    int32_t	 mAttachedBuffers[MAX_VERTEX_ATTRIBS];//重置为-1，因此没使用 GLuint
    uint32_t mBufferOffsets[MAX_VERTEX_ATTRIBS];
    uint32_t mActiveIndexBuffer;
    uint32_t mAttachedBufferCount;// todo: 最多可绑定 MAX_VERTEX_ATTRIBS 个 vbo??
    uint32_t mFirstVertex;
} GLVAOState;

struct CmdCache // todo 转发gl命令时考虑thread_local gl context状态，而不只是cmd内部的状态，减少状态变更// 多线程如何支持？// Queue创建，CommandPool借用
{
    bool isStarted;
    ...
    uint32_t mFramebuffer; // gl framebuffer name
    ...
    // Pipeline state
    uint32_t			mPipeline; // gl program name
    ...
    // Textures
    Sampler*			pTextureSampler;//gles 2.0 没对应 sampler object，没意义
...
void util_init_extension_support(GLExtensionSupport* pExtensionSupport, const char* availableExtensions)
{
	...
	pExtensionSupport->mHasTextureBorderClampOES = strstr(availableExtensions, "GL_OES_texture_border_clamp") != nullptr;// iPhone XR OpenGL ES 2.0 Metal - 68.8 不支持GL_OES_texture_border_clamp
	...
	// Extension GL_EXT_disjoint_timer_query // iPhone XR OpenGL ES 2.0 Metal - 68.8 不支持GL_EXT_disjoint_timer_query
	...
	pExtensionSupport->mHasMapbufferOES = strstr(availableExtensions, "GL_OES_mapbuffer") != nullptr;// todo es 3.0 glMapBuffer系列是否等同glMapBufferOES？
    ...
    // Extension GL_OES_get_program
//	pExtensionSupport->mHasShaderBinaryOES = strstr(availableExtensions, "GL_OES_get_program") != nullptr;
//
//	// Extension GL_OES_get_program
//	pExtensionSupport->mHasProgramBinaryOES = strstr(availableExtensions, "GL_OES_get_program") != nullptr;
//	glGetIntegerv(GL_NUM_PROGRAM_BINARY_FORMATS_OES, &hardext.prgbin_n);
}
...
inline const char* util_get_enum_string(GLenum value)
{
	switch (value)
	{
		...
		case GL_INVALID_FRAMEBUFFER_OPERATION: return "GL_INVALID_FRAMEBUFFER_OPERATION";// todo GL_INVALID_FRAMEBUFFER_OPERATION The framebuffer object is not complete. The offending command is ignored and has no other side effect than to set the error flag. // todo glClear 错误的值(GL_COLOR_BUFFER_BIT + GL_DEPTH_BUFFER_BIT)触发iOS GL_INVALID_FRAMEBUFFER_OPERATION ，但是http://docs.gl/es3/glClear并不声明此错误
		case GL_FRAMEBUFFER_UNDEFINED_OES: return "GL_FRAMEBUFFER_UNDEFINED"; // todo 3.0 core 改成 GL_FRAMEBUFFER_UNDEFINED GL_FRAMEBUFFER_UNDEFINED is returned if target is the default framebuffer, but the default framebuffer does not exist.
		case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT: return "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE"; //is returned if the value of GL_RENDERBUFFER_SAMPLES is not the same for all attached renderbuffers or, if the attached images are a mix of renderbuffers and textures, the value of GL_RENDERBUFFER_SAMPLES is not zero.
...
#define CHECK_GL_RESULT(exp)                                                               \
	{                                                                                     \
		exp;                                                                              \
		GLenum glRes = glGetError();                                                      \
		if (glRes != GL_NO_ERROR)                                                         \
		{                                                                                 \
		LOGF(eERROR, "%s: FAILED with Error: %x: %s", #exp, glRes, util_get_enum_string(glRes));  \
			ASSERT(false);                                                                \
		}                                                                                 \
	}
...
#endif // defined(ENABLE_GRAPHICS_DEBUG)
...
static inline uint32_t gl_type_byte_size(GLenum type)
{
    switch (type)
    {
        ...
        case GL_INT_VEC3:
        case GL_BOOL_VEC3:
        case GL_FLOAT_VEC3: return 16;//todo 和 msl 一样？？本该是 3x4=12 实际对齐到16
        case GL_INT_VEC4:
        case GL_BOOL_VEC4:
        case GL_FLOAT_VEC4: return 16;
        case GL_FLOAT_MAT2: return 32;// todo 为什么是32
        case GL_FLOAT_MAT3: return 48;// todo 为什么是48
        case GL_FLOAT_MAT4: return 64;
        default: ASSERT(false && "Unknown GL type"); return 0;
    }
}

static inline void util_gl_set_uniform(uint32_t location, uint8_t* data, GLenum type, uint32_t size)//比如uniform float4 uniformBlock[424];，size=424
{
    switch (type)
    {
        case GL_INT:
        case GL_BOOL:
        case GL_SAMPLER_2D:
        case GL_SAMPLER_CUBE: CHECK_GL_RESULT(glUniform1iv(location, size, (GLint*)data)); break;//http://docs.gl/es2/glUniform size: This should be 1 if the targeted uniform variable is not an array, and 1 or more if it is an array.
        case GL_FLOAT: CHECK_GL_RESULT(glUniform1fv(location, size, (GLfloat*)data)); break;
        case GL_INT_VEC2:
        case GL_BOOL_VEC2: CHECK_GL_RESULT(glUniform2iv(location, size, (GLint*)data)); break;
        case GL_FLOAT_VEC2: CHECK_GL_RESULT(glUniform2fv(location, size, (GLfloat*)data)); break;//#168 glUniform2fv(uRootConstants.scaleBias, 1, {0.0011161, -0.0024155})
        case GL_INT_VEC3:
        case GL_BOOL_VEC3: CHECK_GL_RESULT(glUniform3iv(location, size, (GLint*)data)); break;
        case GL_FLOAT_VEC3: CHECK_GL_RESULT(glUniform3fv(location, size, (GLfloat*)data)); break;
        case GL_INT_VEC4:
        case GL_BOOL_VEC4: CHECK_GL_RESULT(glUniform4iv(location, size, (GLint*)data)); break;
        case GL_FLOAT_VEC4: CHECK_GL_RESULT(glUniform4fv(location, size, (GLfloat*)data)); break;//#166 glUniform4fv(uRootConstants.color, 1, {1.0000000, 1.0000000, 0.0000000, 1.0000000})
        case GL_FLOAT_MAT2: CHECK_GL_RESULT(glUniformMatrix2fv(location, size, GL_FALSE, (GLfloat*)data)); break;
        case GL_FLOAT_MAT3: CHECK_GL_RESULT(glUniformMatrix3fv(location, size, GL_FALSE, (GLfloat*)data)); break;
        case GL_FLOAT_MAT4: CHECK_GL_RESULT(glUniformMatrix4fv(location, size, GL_FALSE, (GLfloat*)data)); break;//#162 glUniformMatrix4fv(uniformBlock_rootcbv, 1, 0, {...})
...
{
    GLint infoLen = 0;
    CHECK_GL_RESULT(glGetProgramiv(program, GL_INFO_LOG_LENGTH, &infoLen));
    if (infoLen > 1)
    {
        char* infoLog = nullptr;
        infoLog = (char*)tf_calloc(infoLen + 1, sizeof(char));
        CHECK_GL_RESULT(glGetProgramInfoLog(program, infoLen + 1, nullptr, infoLog));
...
{
    CHECK_GL_RESULT(glLinkProgram(program));
    GLint status = GL_FALSE;
    CHECK_GL_RESULT(glGetProgramiv(program, GL_LINK_STATUS, &status));
    if (status)
    {
        /// glValidateProgram 会验证当前 framebuffer 是否满足要求，从而产生意外的 status /// TODO 注释掉以下几行
        CHECK_GL_RESULT(glValidateProgram(program));
        CHECK_GL_RESULT(glGetProgramiv(program, GL_VALIDATE_STATUS, &status));
        if (!status)
        {
...
static bool addDevice(Renderer* pRenderer, const RendererDesc* pDesc)
{
...
#endif//defined(VULKAN) && defined(ANDROID)
    ...
    GLint nShaderBinaryFormats = 0;//TODO
    GLint nProgramBinaryFormats = 0;//TODO
    ...
    glGetIntegerv(GL_NUM_SHADER_BINARY_FORMATS, &nShaderBinaryFormats);//todo 华为 mali gpu 返回1
    glGetIntegerv(GL_NUM_PROGRAM_BINARY_FORMATS_OES, &nProgramBinaryFormats);//todo 华为 mali gpu 返回1
    ...
    CHECK_GL_RESULT(glPixelStorei(GL_PACK_ALIGNMENT, packAlignment));
    CHECK_GL_RESULT(glPixelStorei(GL_UNPACK_ALIGNMENT, unpackAlignment));
    ...
    pRenderer->pActiveGpuSettings->mMultiDrawIndirect = 0; // 3.1 core 支持 glDrawArraysIndirect
    ...
    LOGF(LogLevel::eINFO, "Extensions: %s", glExtensions);// 华为 GL_EXT_debug_marker GL_ARM_rgba8 GL_ARM_mali_shader_binary GL_OES_depth24 GL_OES_depth_texture GL_OES_depth_texture_cube_map GL_OES_packed_depth_stencil GL_OES_rgb8_rgba8 GL_EXT_read_format_bgra GL_OES_compressed_paletted_texture GL_OES_compressed_ETC1_RGB8_texture GL_OES_standard_derivatives GL_OES_EGL_image GL_OES_EGL_image_external GL_OES_EGL_image_external_essl3 GL_OES_EGL_sync GL_OES_texture_npot GL_OES_vertex_half_float GL_OES_required_internalformat GL_OES_vertex_array_object GL_OES_mapbuffer GL_EXT_texture_format_BGRA8888 GL_EXT_texture_rg GL_EXT_texture_type_2_10_10_10_REV GL_OES_fbo_render_mipmap GL_OES_element_index_uint GL_EXT_shadow_samplers GL_OES_texture_compression_astc GL_KHR_texture_compression_astc_ldr GL_KHR_texture_compression_astc_hdr GL_KHR_texture_compression_astc_sliced_3d GL_KHR_debug GL_EXT_occlusion_query_boolean GL_EXT_disjoint_timer_query GL_EXT_blend_minmax GL_EXT_discard_framebuffer GL_OES_get_program_binary GL_OES_texture_3D GL_EXT_texture_storage GL_EXT_multisampled_render_t
    ...
    const char* ptr = strtok(glExtensions, " ");//todo 拆成map，ppDeviceExtensions元素为key，找个数 dawn src/dawn_native/opengl/OpenGLFunctions.cpp std::unordered_set<std::string> mSupportedGLExtensionsSet;  mSupportedGLExtensionsSet.insert(extensionName); const char* ptr = strtok(glExtensions, " ");
    while (ptr != nullptr)
    {
...
void gl_initRenderer(const char* appName, const RendererDesc* pDesc, Renderer** ppRenderer)
{
    ...
    LOGF(LogLevel::eINFO, "Using OpenGL ES 2.0"); // todo 此时不应该打印es 2.0，对于我们可以是2.0 3.x
...
void gl_addQueue(Renderer* pRenderer, QueueDesc* pDesc, Queue** ppQueue)
{
    ...
    pQueue->mGLES.pCmdCache->mDepthRange = vec2(0, 1);//采用 Metal 坐标系//gles 默认[-1, 1]，todo 这里直接改成[0, 1] shader 也适配了？
...
void gl_addCmd(Renderer* pRenderer, const CmdDesc* pDesc, Cmd** ppCmd)
{
    ...
    pCmd->pQueue = pDesc->pPool->pQueue;//todo 应该主动增加引用计数？
    pCmd->mGLES.pCmdPool = pDesc->pPool;//todo 应该主动增加引用计数？
...
void gl_addRenderTarget(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget)
{
    ...
    else
    {
        ...
        CHECK_GL_RESULT(glGenFramebuffers(1, &pRenderTarget->mGLES.mFramebuffer));
    }
    ...
    if (!pRenderer->pCapBits->canRenderTargetWriteTo[pDesc->mFormat])
    {
        ...
        LOGF(LogLevel::eWARNING, "RenderTarget format \"%s\" is not supported, falling back to \"%s\"", TinyImageFormat_Name(pDesc->mFormat), TinyImageFormat_Name(targetFormat));//RenderTarget format "D32_SFLOAT" is not supported, falling back to "D16_UNORM"
    }

    if (gExtensionSupport.mHasDepthTextureOES || !isDepth)//TODO DepthTextureOES 有什么优势，相比 Renderbuffer
    {
    ...
    else
    {
        ...
        {
            CHECK_GL_RESULT(glGenRenderbuffers(1, &pRenderTarget->mGLES.mDepthTarget));
            CHECK_GL_RESULT(glBindRenderbuffer(GL_RENDERBUFFER, pRenderTarget->mGLES.mDepthTarget));
            CHECK_GL_RESULT(glRenderbufferStorage(GL_RENDERBUFFER, internalFormat, pDesc->mWidth, pDesc->mHeight));
            CHECK_GL_RESULT(glBindRenderbuffer(GL_RENDERBUFFER, 0));
        }

        if (pRenderTarget->mGLES.mType & GL_STENCIL_BUFFER_BIT)
        {
            CHECK_GL_RESULT(glGenRenderbuffers(1, &pRenderTarget->mGLES.mStencilTarget));
            CHECK_GL_RESULT(glBindRenderbuffer(GL_RENDERBUFFER, pRenderTarget->mGLES.mStencilTarget));
            CHECK_GL_RESULT(glRenderbufferStorage(GL_RENDERBUFFER, targetFormat != TinyImageFormat_D16_UNORM ? internalFormat : GL_STENCIL_INDEX8, pDesc->mWidth, pDesc->mHeight));
            CHECK_GL_RESULT(glBindRenderbuffer(GL_RENDERBUFFER, 0));
        }
    }
...
void gl_removeRenderTarget(Renderer* pRenderer, RenderTarget* pRenderTarget)
{
    ...
    if (pRenderTarget->mGLES.mDepthTarget)
        CHECK_GL_RESULT(glDeleteRenderbuffers(1, &pRenderTarget->mGLES.mDepthTarget));
    if (pRenderTarget->mGLES.mStencilTarget)
        CHECK_GL_RESULT(glDeleteRenderbuffers(1, &pRenderTarget->mGLES.mStencilTarget));
    if (pRenderTarget->mGLES.mFramebuffer)
        CHECK_GL_RESULT(glDeleteFramebuffers(1, &pRenderTarget->mGLES.mFramebuffer));
        ...
        bool, uint32_t macroCount, ShaderMacro* pMacros, BinaryShaderStageDesc* pOut, const char* pEntryPoint)
{
    ...
    if (version)
    {
        codeOffset = (version - code);// 找出#version的#位置
        outCodeSize += codeSize - codeOffset; // 减去#vesion前面的注释？
    }
    else
    {
    ...
    memcpy(outCode + outCodeOffset, (void*)(code + codeOffset), codeSize - codeOffset);//删了前面的注释，保留#version开始的有效内容
    ...
    CHECK_GL_RESULT(glShaderSource(shader, 1, (const GLchar*const*)&outCode, (GLint*)&outCodeSize));
    CHECK_GL_RESULT(glCompileShader(shader));

    // Check the compile status
    CHECK_GL_RESULT(glGetShaderiv(shader, GL_COMPILE_STATUS, &compiled));
    if (!compiled)
    {
        GLint infoLen = 0;
        CHECK_GL_RESULT(glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &infoLen));
        if (infoLen > 1)
        {
            char* infoLog = nullptr;
            infoLog = (char*)tf_calloc(infoLen + 1, sizeof(char));
            CHECK_GL_RESULT(glGetShaderInfoLog(shader, infoLen, nullptr, (GLchar*)infoLog));
...
void gl_addShaderBinary(Renderer* pRenderer, const BinaryShaderDesc* pDesc, Shader** ppShaderProgram)
{
    ...
    for (uint32_t i = 0; i < SHADER_STAGE_COUNT; ++i)
    {
        ...
        CHECK_GL_RESULT(glAttachShader(pShaderProgram->mGLES.mProgram, pStage->mShader));
        CHECK_GL_RESULT(glDeleteShader(pStage->mShader));//TODO glDeleteShader的正确用法是什么，无需先detach？
    }

    // Validate GL shader program
    if (!util_link_and_validate_program(pShaderProgram->mGLES.mProgram))
    {
        CHECK_GL_RESULT(glDeleteProgram(pShaderProgram->mGLES.mProgram));
        ASSERT(false);
        return;
    }
...
void gl_addBuffer(Renderer* pRenderer, const BufferDesc* pDesc, Buffer** ppBuffer)
{
    ...
    if ((pDesc->mDescriptors & DESCRIPTOR_TYPE_INDEX_BUFFER) || (pDesc->mDescriptors & DESCRIPTOR_TYPE_VERTEX_BUFFER))
    {
        ...
        if (usage != GL_NONE)
        {
            CHECK_GL_RESULT(glGenBuffers(1, &pBuffer->mGLES.mBuffer));
            CHECK_GL_RESULT(glBindBuffer(pBuffer->mGLES.mTarget, pBuffer->mGLES.mBuffer));
            CHECK_GL_RESULT(glBufferData(pBuffer->mGLES.mTarget, pDesc->mSize, NULL, usage));
            CHECK_GL_RESULT(glBindBuffer(pBuffer->mGLES.mTarget, GL_NONE));
        }
...
void gl_removeBuffer(Renderer* pRenderer, Buffer* pBuffer)
{
    ...
    {
        CHECK_GL_RESULT(glDeleteBuffers(1, &pBuffer->mGLES.mBuffer));
    }
...
void gl_unmapBuffer(Renderer* pRenderer, Buffer* pBuffer)
{
    ...
    if (pBuffer->mMemoryUsage != RESOURCE_MEMORY_USAGE_CPU_ONLY)
    {
        if (gExtensionSupport.mHasMapbufferOES)
        {
            CHECK_GL_RESULT(glUnmapBufferOES(pBuffer->mGLES.mTarget));
        }
        else
        {
            CHECK_GL_RESULT(glBufferSubData(pBuffer->mGLES.mTarget, 0, pBuffer->mSize, pBuffer->mGLES.pGLCpuMappedAddress));
        }
    }
}
...
void gl_addTexture(Renderer* pRenderer, const TextureDesc* pDesc, Texture** ppTexture)
{
    ...
    if (!pRenderer->pCapBits->canShaderReadFrom[format])
    {
        ...
        {
            LOGF(LogLevel::eERROR, "Compressed format \"%s\" is not supported!", TinyImageFormat_Name(format));// iPhone XR ES2.0 ctx 报错 Compressed format "ASTC_8x8_UNORM" is not supported! ES3.0正常 https://zhuanlan.zhihu.com/p/158740249
            return;
        }
    ...
    CHECK_GL_RESULT(glGenTextures(1, &pTexture->mGLES.mTexture));
    ...
    {
        CHECK_GL_RESULT(glBindTexture(pTexture->mGLES.mTarget, pTexture->mGLES.mTexture));
        //TODO Set all miplevels
        CHECK_GL_RESULT(glTexImage2D(pTexture->mGLES.mTarget, 0, pTexture->mGLES.mGlFormat,
                                     pTexture->mWidth, pTexture->mHeight, 0, pTexture->mGLES.mGlFormat, pTexture->mGLES.mType, NULL));
        CHECK_GL_RESULT(glBindTexture(pTexture->mGLES.mTarget, GL_NONE));
...
void gl_removeTexture(Renderer* pRenderer, Texture* pTexture)
{
    ...
    {
        CHECK_GL_RESULT(glDeleteTextures(1, &pTexture->mGLES.mTexture));
    }
...
void gl_addRootSignature(Renderer* pRenderer, const RootSignatureDesc* pRootSignatureDesc, RootSignature** ppRootSignature)
{
    ...
    DescriptorIndexMap                                       indexMap;//todo 放pRes->name和pRes在shaderResources的位置？

    uint32_t uniqueTextureCount = 0;
    /*
     2021-06-14 21:20:59 [MainThread     ] GLESShaderReflection.cp:200    DBG| Program 3, Uniform 6, location: 0, size 4, type 35666, name "uniformBlock[0]"
     */
    ...
    for (uint32_t sh = 0; sh < pRootSignatureDesc->mShaderCount; ++sh)
    {
        ...
        {
            LOGF(LogLevel::eERROR, "Compute shader not supported on OpenGL ES 2.0");//todo 如果支持3.1，改变日志输出
        }
        ...
        for (uint32_t i = 0; i < pReflection->mShaderResourceCount; ++i)
        {
            ...
            if (pNode == indexMap.mMap.end())
            {
                ...
                switch (pRes->type)
                {
                    ...
                    {
                        for (uint32_t v = 0; v < pReflection->mVariableCount; ++v)//todo 如何理解
                        {
            ...
            else
            {
                ...
                {
                    if(shaderResources[pNode->second].set != pRes->set)// TODO uniformBlock 第一次走这
                    {
                        LOGF(eERROR,
                    ...
                    shaderResources[pNode->second].size = max(shaderResources[pNode->second].size, pRes->size);//todo uniform float4 uniformBlock[424];变成4?
                }
            }
        }
    }
    ...
    eastl::sort(shaderVariables.begin(), shaderVariables.end(), util_compare_shader_variable);//todo 为什么要排序

    RootSignature* pRootSignature = (RootSignature*)tf_calloc_memalign(1, alignof(RootSignature), totalSize);// todo totalSize=440, alignof(RootSignature)=8，在此分配440/8=55个RootSignature??
    ...
    for (uint32_t i = 0; i < pRootSignature->mGLES.mVariableCount; ++i)
    {
        ...
        LOGF(LogLevel::eDEBUG, "%u: Shader Variable \"%s\" offset: %u", i, pRes->name, pRes->offset);//TODO 0: Shader Variable "uRootConstants.color" offset: 0 //1: Shader Variable "uRootConstants.scaleBias" offset: 1
    ...
    // Only one sampler can be applied in a rootSignature for GLES//todo 为什么？平台限制还是结构约束？
...
void addGraphicsPipeline(Renderer* pRenderer, const GraphicsPipelineDesc* pDesc, Pipeline** ppPipeline)
{
    ...
    CHECK_GL_RESULT(glUseProgram(pShaderProgram->mGLES.mProgram));
...
void gl_removePipeline(Renderer* pRenderer, Pipeline* pPipeline)
{
    ...
    if (pPipeline->mGLES.pVAOState)
    {
        for (uint32_t stateIndex = 0; stateIndex < VAO_STATE_CACHE_SIZE; ++stateIndex)
        {
            ...
            {
                CHECK_GL_RESULT(glDeleteVertexArraysOES(1, &pPipeline->mGLES.pVAOState[stateIndex].mVAO));
            }
        }
    }
...
//todo struct 数据源与 uniform 如何匹配，gles 2.0如何做到整个 uniform 更新？ block 字段偏移如何实现？
typedef struct BufferDescriptorHandle
{
    void* pBuffer;
    uint32_t mOffset;//todo 如何写入
...
void gl_endCmd(Cmd* pCmd)
{
    ...
    {
        CHECK_GL_RESULT(glBindVertexArrayOES(GL_NONE));
        ...
        uint32_t depthMipSlice)
{
    ...
    {
        // Reset
        if (cmdCache->mFramebuffer)// mFramebuffer 有值，而外部传入 nullptr color, depth stencil，需要重置为 default framebuffer
        {
            CHECK_GL_RESULT(glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE));
            cmdCache->mFramebuffer = GL_NONE;
            getGLSurfaceSize(NULL, &pCmd->mGLES.pCmdPool->pCmdCache->mRenderTargetHeight);//viewport 翻转 Y 使用此值
        }
        return;
    }
    ...
    for (uint32_t rtIndex = 0; rtIndex < renderTargetCount; ++rtIndex)
    {
        ...
        if (ppRenderTargets[rtIndex]->pTexture)
        {
            ...
            {
                CHECK_GL_RESULT(glBindFramebuffer(GL_FRAMEBUFFER, ppRenderTargets[rtIndex]->mGLES.mFramebuffer));
                cmdCache->mFramebuffer = ppRenderTargets[rtIndex]->mGLES.mFramebuffer;
            }
            //  渲染到 CUBE 使用 GL_TEXTURE_CUBE_MAP_POSITIVE_X ... GL_TEXTURE_CUBE_MAP_NEGATIVE_Z
            //  MTR 用法是 GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2...
            CHECK_GL_RESULT(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + rtIndex, GL_TEXTURE_2D, ppRenderTargets[rtIndex]->pTexture->mGLES.mTexture, 0));
        }
        ...
        if (pLoadActions && pLoadActions->mLoadActionsColor[rtIndex] == LOAD_ACTION_CLEAR)
        {
            ...
            {
                CHECK_GL_RESULT(glClearColor(clearColor[0], clearColor[1], clearColor[2], clearColor[3]));//TODO gles 支持清理指定的 attachment 吗？不支持的话，是否加上 if (rtIndex == 0)判断，跳过 glClearColor 重复配置？
    ...
    // todo 性能优化：使用 glDiscardFramebufferEXT
    if (pDepthStencil && pDepthStencil->mGLES.mType & GL_DEPTH_BUFFER_BIT)
    {
        if (cmdCache->mFramebuffer)
        {
            ...
            {
                CHECK_GL_RESULT(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, pDepthStencil->pTexture->mGLES.mTexture, 0));
            }
            else
            {
                CHECK_GL_RESULT(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, pDepthStencil->mGLES.mDepthTarget));
            }
        }
        ...
        if (pLoadActions && pLoadActions->mLoadActionDepth == LOAD_ACTION_CLEAR)
        {
            ...
            {
                CHECK_GL_RESULT(glClearDepthf(pLoadActions->mClearDepth.depth));
            ...
            if (!cmdCache->mDepthStencilState.mDepthWrite)
            {
                ...
                CHECK_GL_RESULT(glDepthMask(GL_TRUE));
    ...
    if (pDepthStencil && pDepthStencil->mGLES.mType & GL_STENCIL_BUFFER_BIT)
    {
        if (cmdCache->mFramebuffer)
        {
            if (pDepthStencil->pTexture)
            {
                CHECK_GL_RESULT(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, pDepthStencil->pTexture->mGLES.mTexture, 0));
            }
            else
            {
                CHECK_GL_RESULT(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, pDepthStencil->mGLES.mStencilTarget));
            }
        }
    ...
    if (clearMask)
    {
        CHECK_GL_RESULT(glClear(clearMask));
    }
}
...
void gl_cmdSetScissor(Cmd* pCmd, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    {
        CHECK_GL_RESULT(glScissor(x, yOffset, width, height));
...
void gl_cmdSetStencilReferenceValue(Cmd* pCmd, uint32_t val)
{
    ...
    if (cmdCache->mStencilRefValue != val)
    {
        ...
        {
            CHECK_GL_RESULT(glStencilFunc(cmdCache->mDepthStencilState.mStencilFrontFunc, val, cmdCache->mDepthStencilState.mStencilReadMask));
        }
        else
        {
            CHECK_GL_RESULT(glStencilFuncSeparate(GL_FRONT, cmdCache->mDepthStencilState.mStencilFrontFunc, val, cmdCache->mDepthStencilState.mStencilReadMask));
            CHECK_GL_RESULT(glStencilFuncSeparate(GL_BACK, cmdCache->mDepthStencilState.mStencilBackFunc, val, cmdCache->mDepthStencilState.mStencilReadMask));
        }
...
void gl_cmdBindPipeline(Cmd* pCmd, Pipeline* pPipeline)
{
    ...
    {
        CHECK_GL_RESULT(glUseProgram(glProgram));
    ...
    for (uint32_t vertexAttribIndex = 0; vertexAttribIndex < MAX_VERTEX_ATTRIBS; ++vertexAttribIndex)
    {
        if (shouldEnable[vertexAttribIndex])
        {
            if (!cmdCache->mVertexAttribCache[vertexAttribIndex].mIsActive)
            {
                if (!gExtensionSupport.mHasVertexArrayObjectOES)
                {
                    CHECK_GL_RESULT(glEnableVertexAttribArray(vertexAttribIndex));
                }
        ...
        else if (cmdCache->mVertexAttribCache[vertexAttribIndex].mIsActive)
        {
            if (!gExtensionSupport.mHasVertexArrayObjectOES)
            {
                CHECK_GL_RESULT(glDisableVertexAttribArray(vertexAttribIndex));
            }
    ...
    if (cmdCache->mRasterizerState.mCullMode != pPipeline->mGLES.pRasterizerState->mCullMode)
    {
        ...
        {
            CHECK_GL_RESULT(glEnable(GL_CULL_FACE));
            CHECK_GL_RESULT(glCullFace(pPipeline->mGLES.pRasterizerState->mCullMode));
        }
        else
        {
            CHECK_GL_RESULT(glDisable(GL_CULL_FACE));
        }
    ...
    if (pPipeline->mGLES.pRasterizerState->mCullMode)
    {
        ...
        {
            CHECK_GL_RESULT(glFrontFace(pPipeline->mGLES.pRasterizerState->mFrontFace));
    ...
    {
        CHECK_GL_RESULT(pPipeline->mGLES.pRasterizerState->mScissorTest ? glEnable(GL_SCISSOR_TEST) : glDisable(GL_SCISSOR_TEST));
    ...
    {
        CHECK_GL_RESULT(pPipeline->mGLES.pDepthStencilState->mDepthTest ? glEnable(GL_DEPTH_TEST) : glDisable(GL_DEPTH_TEST));
    ...
    if (pPipeline->mGLES.pDepthStencilState->mDepthTest)
    {
        ...
        {
            CHECK_GL_RESULT(glDepthMask(pPipeline->mGLES.pDepthStencilState->mDepthWrite));
        ...
        {
            CHECK_GL_RESULT(glDepthFunc(pPipeline->mGLES.pDepthStencilState->mDepthFunc));
    ...
    {
        CHECK_GL_RESULT(pPipeline->mGLES.pDepthStencilState->mStencilTest ? glEnable(GL_STENCIL_TEST) : glDisable(GL_STENCIL_TEST));
    ...
    if(pPipeline->mGLES.pDepthStencilState->mStencilTest)
    {
        ...
        {
            CHECK_GL_RESULT(glStencilFuncSeparate(GL_FRONT, pPipeline->mGLES.pDepthStencilState->mStencilFrontFunc, cmdCache->mStencilRefValue, pPipeline->mGLES.pDepthStencilState->mStencilReadMask));
        ...
        {
            CHECK_GL_RESULT(glStencilOpSeparate(GL_FRONT, pPipeline->mGLES.pDepthStencilState->mStencilFrontFail, pPipeline->mGLES.pDepthStencilState->mDepthFrontFail, pPipeline->mGLES.pDepthStencilState->mStencilFrontPass));
        ...
        {
            CHECK_GL_RESULT(glStencilFuncSeparate(GL_BACK, pPipeline->mGLES.pDepthStencilState->mStencilBackFunc, cmdCache->mStencilRefValue, pPipeline->mGLES.pDepthStencilState->mStencilReadMask));
        ...
        {
            CHECK_GL_RESULT(glStencilOpSeparate(GL_BACK, pPipeline->mGLES.pDepthStencilState->mStencilBackFail, pPipeline->mGLES.pDepthStencilState->mDepthBackFail, pPipeline->mGLES.pDepthStencilState->mStencilBackPass));
        ...
        {
            CHECK_GL_RESULT(glStencilMask(pPipeline->mGLES.pDepthStencilState->mStencilWriteMask));
    ...
    {
        CHECK_GL_RESULT(pPipeline->mGLES.pBlendState->mBlendEnable ? glEnable(GL_BLEND) : glDisable(GL_BLEND));//todo 绑定时修改状态还是延迟到 DrawXyz 再修改所有受影响的状态？
    ...
    if (pPipeline->mGLES.pBlendState->mBlendEnable)
    {
        ...
        {
            CHECK_GL_RESULT(glBlendFuncSeparate(pPipeline->mGLES.pBlendState->mSrcRGBFunc, pPipeline->mGLES.pBlendState->mDstRGBFunc, pPipeline->mGLES.pBlendState->mSrcAlphaFunc, pPipeline->mGLES.pBlendState->mDstAlphaFunc));
        ...
        {
            CHECK_GL_RESULT(glBlendEquationSeparate(pPipeline->mGLES.pBlendState->mModeRGB, pPipeline->mGLES.pBlendState->mModeAlpha));
...
void gl_cmdBindDescriptorSet(Cmd* pCmd, uint32_t index, DescriptorSet* pDescriptorSet)
{
    ...
    for (uint32_t sh = 0; sh < pRootSignature->mGLES.mProgramCount; ++sh)
    {
        ...
        if (cmdCache->mPipeline != glProgram)
        {
            CHECK_GL_RESULT(glUseProgram(glProgram));//01_Transformations 存在多次调用，比如 #14 glUseProgram(9) #15 GL_NO_ERROR = glGetError() #16 glUseProgram(6)
            cmdCache->mPipeline = glProgram;
        }
                ...
                case DESCRIPTOR_TYPE_TEXTURE_CUBE:
                {
                    ...
                    for (uint32_t arr = 0; arr < descInfo->mSize; ++arr)
                    {
                        ...
                        if (textureHandle->mTexture != GL_NONE)
                        {
                            ...
                            {
                                CHECK_GL_RESULT(glActiveTexture(GL_TEXTURE0 + textureIndex));
                                cmdCache->mActiveTexture = textureIndex;
                            }

                            if (textureHandle->mStateModified || textureHandle->mTexture != cmdCache->mBoundTexture )
                            {
                                CHECK_GL_RESULT(glBindTexture(target, textureHandle->mTexture));
                            ...
                            {
                                CHECK_GL_RESULT(glTexParameteri(target, GL_TEXTURE_MIN_FILTER, textureHandle->hasMips ? pSampler->mGLES.mMipMapMode : pSampler->mGLES.mMinFilter));
                            ...
                            {
                                CHECK_GL_RESULT(glTexParameteri(target, GL_TEXTURE_MAG_FILTER, pSampler->mGLES.mMagFilter));
                            ...
                            {
                                CHECK_GL_RESULT(glTexParameteri(target, GL_TEXTURE_WRAP_S, pSampler->mGLES.mAddressS));
                            ...
                            {
                                CHECK_GL_RESULT(glTexParameteri(target, GL_TEXTURE_WRAP_T, pSampler->mGLES.mAddressT));
    ...
    {
        CHECK_GL_RESULT(glUseProgram(currentGlProgram));
...
void gl_cmdBindIndexBuffer(Cmd* pCmd, Buffer* pBuffer, uint32_t indexType, uint64_t offset)
{
    ...
    if (pBuffer->mGLES.mBuffer)
    {
        if (pBuffer->mGLES.mBuffer != cmdCache->mActiveIndexBuffer)
        {
            if (!gExtensionSupport.mHasVertexArrayObjectOES)
            {
                CHECK_GL_RESULT(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pBuffer->mGLES.mBuffer));
            }
...
void util_bind_vertex_attribute_buffers(CmdCache* pCmdCache, uint32_t firstVertex, uint16_t vaoIndex)
{
    ...
    for (uint32_t index = 0; index < pCmdCache->mVertexBufferCount; ++index)
    {
        ...
        {
            CHECK_GL_RESULT(glBindBuffer(GL_ARRAY_BUFFER, pCmdCache->mVertexBuffers[index]->mGLES.mBuffer));
        ...
        while (vertexStrideCount < pCmdCache->mVertexBufferStrides[index])
        {
            ...
            if (vertexAttrib->mIndex >= 0)
            {
                if (gExtensionSupport.mHasVertexArrayObjectOES) // Should pass only once per new VAO
                {
                    if (useGPUResource)
                    {
                        // 减少 while() 带来的 glBindBuffer 冗余调用
                        if (pCmdCache->mActiveVertexBuffer != pCmdCache->mVertexBuffers[index]->mGLES.mBuffer) {
                            CHECK_GL_RESULT(glBindBuffer(GL_ARRAY_BUFFER, pCmdCache->mVertexBuffers[index]->mGLES.mBuffer));
                            pCmdCache->mActiveVertexBuffer = pCmdCache->mVertexBuffers[index]->mGLES.mBuffer;
                        }
//						CHECK_GL_RESULT(glBindBuffer(GL_ARRAY_BUFFER, pCmdCache->mVertexBuffers[index]->mGLES.mBuffer));//todo 会绑定同一个VBO
                        CHECK_GL_RESULT(glVertexAttribPointer(vertexAttrib->mIndex, vertexAttrib->mSize, vertexAttrib->mType, vertexAttrib->mNormalized,
                    ...
                    CHECK_GL_RESULT(glEnableVertexAttribArray(vertexAttrib->mIndex));
                }
...
void util_reset_vao(GLVAOState* pState)
{
    ...
    CHECK_GL_RESULT(glDeleteVertexArraysOES(1, &pState->mVAO));

    pState->mActiveIndexBuffer = GL_NONE;// VAO 记录 Element Array Buffer (即 Index Buffer)和 Vertex Attribute Array(即 Vertex Buffer)
    for (uint32_t vertexAttribLocation = 0; vertexAttribLocation < MAX_VERTEX_ATTRIBS; ++vertexAttribLocation)//todo 为什么不记录激活的 VAO数量，MAX_VERTEX_ATTRIBS 有些粗糙
    {
...
void util_bind_vao(CmdCache* pCmdCache, uint32_t firstVertex, bool isIndexed)
{
    ...
    {
        if (index == VAO_STATE_CACHE_SIZE)//todo VAO 数量对内存，性能的影响
        {
        ...
        CHECK_GL_RESULT(glGenVertexArraysOES(1, &pCmdCache->pActiveVAOStates[index].mVAO));
        CHECK_GL_RESULT(glBindVertexArrayOES(pCmdCache->pActiveVAOStates[index].mVAO));
        ...
        if (isIndexed)
        {
            CHECK_GL_RESULT(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pCmdCache->mActiveIndexBuffer));
        }
...
void gl_cmdDraw(Cmd* pCmd, uint32_t vertexCount, uint32_t firstVertex)
{
    ...
    CHECK_GL_RESULT(glDrawArrays(cmdCache->mGlPrimitiveTopology, firstVertex, vertexCount));
}
...
void gl_cmdDrawIndexed(Cmd* pCmd, uint32_t indexCount, uint32_t firstIndex, uint32_t firstVertex)
{
    ...
    {
        CHECK_GL_RESULT(glDrawElements(cmdCache->mGlPrimitiveTopology, indexCount, GL_UNSIGNED_SHORT, (void*)(uintptr_t)offset));
    }
    else // Use CPU resource
    {
        CHECK_GL_RESULT(glDrawElements(cmdCache->mGlPrimitiveTopology, indexCount, GL_UNSIGNED_SHORT, (void*)((uint8_t*)cmdCache->pIndexBuffer->mGLES.pGLCpuMappedAddress + offset)));
    }
}
...
void gl_cmdUpdateBuffer(Cmd* pCmd, Buffer* pBuffer, uint64_t dstOffset, Buffer* pSrcBuffer, uint64_t srcOffset, uint64_t size)
{
    ...
    ASSERT(pBuffer);
    ASSERT(pCmd->mGLES.pCmdPool->pCmdCache->isStarted);
    GLboolean ret = glIsBuffer(pBuffer->mGLES.mBuffer);
    printf("glIsBuffer(%d) -> %d\n", pBuffer->mGLES.mBuffer, ret);
    CHECK_GL_RESULT(glBindBuffer(pBuffer->mGLES.mTarget, pBuffer->mGLES.mBuffer));
    CHECK_GL_RESULT(glBufferSubData(pBuffer->mGLES.mTarget, dstOffset, size, (uint8_t*)pSrcBuffer->pCpuMappedAddress + srcOffset));
    CHECK_GL_RESULT(glBindBuffer(pBuffer->mGLES.mTarget, GL_NONE));
}
...
void gl_cmdUpdateSubresource(Cmd* pCmd, Texture* pTexture, Buffer* pSrcBuffer, const SubresourceDataDesc* pSubresourceDesc)
{
    ...
    if (pTexture->mGLES.mType == GL_NONE) // Compressed image
    {
        ...
        CHECK_GL_RESULT(glCompressedTexImage2D(target, pSubresourceDesc->mMipLevel, pTexture->mGLES.mInternalFormat,
                                               width, height, 0, imageByteSize, (uint8_t*)pSrcBuffer->pCpuMappedAddress + pSubresourceDesc->mSrcOffset));
    }
    else
    {
        CHECK_GL_RESULT(glTexImage2D(target, pSubresourceDesc->mMipLevel, pTexture->mGLES.mGlFormat,
        ...
        Queue* pQueue, const QueueSubmitDesc* pDesc)
{
    ...
    if (ppCmds[0]->mGLES.pCmdPool->pCmdCache->mRasterizerState.mScissorTest)
    {
        ...
        CHECK_GL_RESULT(glDisable(GL_SCISSOR_TEST)); //todo 如果一个 frame 内多次 submit ，若这是第一次 submit，离 Swap buffer 还有距离，不就出 bug 了？
...
void gl_getFenceStatus(Renderer* pRenderer, Fence* pFence, FenceStatus* pFenceStatus)
{
    ...
    {
        *pFenceStatus = FENCE_STATUS_NOT_SUBMITTED;
    }
}
...
void gl_waitForFences(Renderer* pRenderer, uint32_t fenceCount, Fence** ppFences)
{
    ...
    for (uint32_t i = 0; i < fenceCount; ++i)
    {
        ...
        {
            waitForGL = true;// todo 为什么不是因为任何一个 ppFences[i]->mGLES.mSubmitted = true 才设置 waitForGL = true 呢？
        }
    }

    if (waitForGL)
    {
        CHECK_GL_RESULT(glFinish());
    }
}
...
void gl_addQueryPool(Renderer* pRenderer, const QueryPoolDesc* pDesc, QueryPool** ppQueryPool)
{
    ...
    if (glGenQueriesEXT)
    CHECK_GL_RESULT(glGenQueriesEXT(pDesc->mQueryCount, pQueryPool->mGLES.pQueries));
...
void gl_removeQueryPool(Renderer* pRenderer, QueryPool* pQueryPool)
{
    ...
    CHECK_GL_RESULT(glDeleteQueriesEXT(pQueryPool->mCount, pQueryPool->mGLES.pQueries));
...
void gl_cmdResetQueryPool(Cmd* pCmd, QueryPool* pQueryPool, uint32_t startQuery, uint32_t queryCount)
{
    ...
    // Only available as extension in OpenGL ES 2.0
//	if (glGenQueriesEXT)//  GL_GPU_DISJOINT_EXT iPhoneXR ES 3.0触发GL_INVALID_ENUM glError
//		CHECK_GL_RESULT(glGetIntegerv(GL_GPU_DISJOINT_EXT, &pQueryPool->mGLES.mDisjointOccurred));
}
...
void gl_cmdBeginQuery(Cmd* pCmd, QueryPool* pQueryPool, QueryDesc* pQuery)
{
    ...
    CHECK_GL_RESULT(glQueryCounterEXT(pQueryPool->mGLES.pQueries[pQuery->mIndex], pQueryPool->mGLES.mType));
}
...
void gl_cmdResolveQuery(Cmd* pCmd, QueryPool* pQueryPool, Buffer* pReadbackBuffer, uint32_t startQuery, uint32_t queryCount)
{
    ...
    if (glGetQueryObjectivEXT && queryCount)
    {
        ...
        while (!available) {
            CHECK_GL_RESULT(glGetQueryObjectivEXT(pQueryPool->mGLES.pQueries[startQuery + queryCount -1], GL_QUERY_RESULT_AVAILABLE_EXT, &available));
        }
        ...
        if (!pQueryPool->mGLES.mDisjointOccurred)
        {
            ...
            {
                CHECK_GL_RESULT(glGetQueryObjectui64vEXT(pQueryPool->mGLES.pQueries[i + startQuery], GL_QUERY_RESULT_EXT, (GLuint64*)pReadbackBuffer->mGLES.pGLCpuMappedAddress + i));
            }
        }
    }
}
...
void gl_cmdBeginDebugMarker(Cmd* pCmd, float r, float g, float b, const char* pName)
{
	...
	if (glPushGroupMarkerEXT)
		CHECK_GL_RESULT(glPushGroupMarkerEXT(strlen(pName), pName));
#endif
}
...
void gl_cmdEndDebugMarker(Cmd* pCmd)
{
	...
	if (glPopGroupMarkerEXT)
		CHECK_GL_RESULT(glPopGroupMarkerEXT());
#endif
}
...
void gl_cmdAddDebugMarker(Cmd* pCmd, float r, float g, float b, const char* pName)
{
	...
	if(glInsertEventMarkerEXT)
		CHECK_GL_RESULT(glInsertEventMarkerEXT(strlen(pName), pName));
#endif
}
...
void gl_setBufferName(Renderer* pRenderer, Buffer* pBuffer, const char* pName)
{
	...
	// Only available as extension OpenGL ES 2.0
	if (glLabelObjectEXT && pBuffer->mGLES.mTarget != GL_NONE)// todo 为什么判断 target
		CHECK_GL_RESULT(glLabelObjectEXT(GL_BUFFER_OBJECT_EXT, pBuffer->mGLES.mBuffer, strlen(pName), pName));
#endif
}
...
void gl_setTextureName(Renderer* pRenderer, Texture* pTexture, const char* pName)
{
	...
	// Only available as extension OpenGL ES 2.0
	if (glLabelObjectEXT)
		CHECK_GL_RESULT(glLabelObjectEXT(GL_TEXTURE, pTexture->mGLES.mTexture, strlen(pName), pName));
#endif
}
...
void gl_setRenderTargetName(Renderer* pRenderer, RenderTarget* pRenderTarget, const char* pName)
{
	...
	{
		if (pRenderTarget->mGLES.mDepthTarget)
			CHECK_GL_RESULT(glLabelObjectEXT(GL_RENDERBUFFER, pRenderTarget->mGLES.mDepthTarget, strlen(pName), pName));

		if (pRenderTarget->mGLES.mStencilTarget)
			CHECK_GL_RESULT(glLabelObjectEXT(GL_RENDERBUFFER, pRenderTarget->mGLES.mStencilTarget, strlen(pName), pName));
	}
...
void gl_setPipelineName(Renderer* pRenderer, Pipeline* pPipeline, const char* pName)
{
	...
	// Only available as extension OpenGL ES 2.0
	if (glLabelObjectEXT)
		CHECK_GL_RESULT(glLabelObjectEXT(GL_PROGRAM_OBJECT_EXT, pPipeline->mGLES.pRootSignature->mGLES.pProgramTargets[pPipeline->mGLES.mRootSignatureIndex], strlen(pName), pName));
#endif
}
...
void initGLESRenderer(ForgeProcTable *procs, const char *appName, const RendererDesc *pSettings, Renderer **ppRenderer)
{
    // API functions
    procs->addFence = gl_addFence;
    procs->removeFence = gl_removeFence;
    procs->addSemaphore = gl_addSemaphore;
    procs->removeSemaphore = gl_removeSemaphore;
    procs->addQueue = gl_addQueue;
    procs->removeQueue = gl_removeQueue;
    procs->addSwapChain = gl_addSwapChain;
    procs->removeSwapChain = gl_removeSwapChain;

    // command pool functions
    procs->addCmdPool = gl_addCmdPool;
    procs->removeCmdPool = gl_removeCmdPool;
    procs->addCmd = gl_addCmd;
    procs->removeCmd = gl_removeCmd;
    procs->addCmd_n = gl_addCmd_n;
    procs->removeCmd_n = gl_removeCmd_n;

    procs->addRenderTarget = gl_addRenderTarget;
    procs->removeRenderTarget = gl_removeRenderTarget;
    procs->addSampler = gl_addSampler;
    procs->removeSampler = gl_removeSampler;

    // Resource Load functions
    procs->addBuffer = gl_addBuffer;
    procs->removeBuffer = gl_removeBuffer;
    procs->mapBuffer = gl_mapBuffer;
    procs->unmapBuffer = gl_unmapBuffer;
    procs->cmdUpdateBuffer = gl_cmdUpdateBuffer;
    procs->cmdUpdateSubresource = gl_cmdUpdateSubresource;
    procs->addTexture = gl_addTexture;
    procs->removeTexture = gl_removeTexture;
    procs->addVirtualTexture = gl_addVirtualTexture;
    procs->removeVirtualTexture = gl_removeVirtualTexture;

    // shader functions
    procs->addShaderBinary = gl_addShaderBinary;
    procs->removeShader = gl_removeShader;

    procs->addRootSignature = gl_addRootSignature;
    procs->removeRootSignature = gl_removeRootSignature;

    // pipeline functions
    procs->addPipeline = gl_addPipeline;
    procs->removePipeline = gl_removePipeline;
    procs->addPipelineCache = gl_addPipelineCache;
    procs->getPipelineCacheData = gl_getPipelineCacheData;
    procs->removePipelineCache = gl_removePipelineCache;

    // Descriptor Set functions
    procs->addDescriptorSet = gl_addDescriptorSet;
    procs->removeDescriptorSet = gl_removeDescriptorSet;
    procs->updateDescriptorSet = gl_updateDescriptorSet;

    // command buffer functions
    procs->resetCmdPool = gl_resetCmdPool;
    procs->beginCmd = gl_beginCmd;
    procs->endCmd = gl_endCmd;
    procs->cmdBindRenderTargets = gl_cmdBindRenderTargets;
    procs->cmdSetShadingRate = gl_cmdSetShadingRate;
    procs->cmdSetViewport = gl_cmdSetViewport;
    procs->cmdSetScissor = gl_cmdSetScissor;
    procs->cmdSetStencilReferenceValue = gl_cmdSetStencilReferenceValue;
    procs->cmdBindPipeline = gl_cmdBindPipeline;
    procs->cmdBindDescriptorSet = gl_cmdBindDescriptorSet;
    procs->cmdBindPushConstants = gl_cmdBindPushConstants;
    procs->cmdBindDescriptorSetWithRootCbvs = gl_cmdBindDescriptorSetWithRootCbvs;
    procs->cmdBindIndexBuffer = gl_cmdBindIndexBuffer;
    procs->cmdBindVertexBuffer = gl_cmdBindVertexBuffer;
    procs->cmdDraw = gl_cmdDraw;
    procs->cmdDrawInstanced = gl_cmdDrawInstanced;
    procs->cmdDrawIndexed = gl_cmdDrawIndexed;
    procs->cmdDrawIndexedInstanced = gl_cmdDrawIndexedInstanced;
    procs->cmdDispatch = gl_cmdDispatch;

    // Transition Commands
    procs->cmdResourceBarrier = gl_cmdResourceBarrier;
    // Virtual Textures
    procs->cmdUpdateVirtualTexture = gl_cmdUpdateVirtualTexture;

    // queue/fence/swapchain functions
    procs->acquireNextImage = gl_acquireNextImage;
    procs->queueSubmit = gl_queueSubmit;
    procs->queuePresent = gl_queuePresent;
    procs->waitQueueIdle = gl_waitQueueIdle;
    procs->getFenceStatus = gl_getFenceStatus;
    procs->waitForFences = gl_waitForFences;
    procs->toggleVSync = gl_toggleVSync;

    procs->getRecommendedSwapchainFormat = gl_getRecommendedSwapchainFormat;

    //indirect Draw functions
    procs->addIndirectCommandSignature = gl_addIndirectCommandSignature;
    procs->removeIndirectCommandSignature = gl_removeIndirectCommandSignature;
    procs->cmdExecuteIndirect = gl_cmdExecuteIndirect;

    /************************************************************************/
    // GPU Query Interface
    /************************************************************************/
    procs->getTimestampFrequency = gl_getTimestampFrequency;
    procs->addQueryPool = gl_addQueryPool;
    procs->removeQueryPool = gl_removeQueryPool;
    procs->cmdResetQueryPool = gl_cmdResetQueryPool;
    procs->cmdBeginQuery = gl_cmdBeginQuery;
    procs->cmdEndQuery = gl_cmdEndQuery;
    procs->cmdResolveQuery = gl_cmdResolveQuery;
    /************************************************************************/
    // Stats Info Interface
    /************************************************************************/
    procs->calculateMemoryStats = gl_calculateMemoryStats;
    procs->calculateMemoryUse = gl_calculateMemoryUse;
    procs->freeMemoryStats = gl_freeMemoryStats;
    /************************************************************************/
    // Debug Marker Interface
    /************************************************************************/
    procs->cmdBeginDebugMarker = gl_cmdBeginDebugMarker;
    procs->cmdEndDebugMarker = gl_cmdEndDebugMarker;
    procs->cmdAddDebugMarker = gl_cmdAddDebugMarker;
    procs->cmdWriteMarker = gl_cmdWriteMarker;
    /************************************************************************/
    // Resource Debug Naming Interface
    /************************************************************************/
    procs->setBufferName = gl_setBufferName;
    procs->setTextureName = gl_setTextureName;
    procs->setRenderTargetName = gl_setRenderTargetName;
    procs->setPipelineName = gl_setPipelineName;

    /************************************************************************/
    // IRay Interface
    /************************************************************************/
//    procs->isRaytracingSupported = gl_isRaytracingSupported;
